...
export function Minimap({ currentIndex, onNavigate }: MinimapProps) {
  const levels = [...ZOOM_LEVELS].reverse(); // outermost first
  const totalLevels = levels.length;

  const mapSize = 270;
  const padding = 12;
  const innerSize = 44; // bigger center so "App" + dot fit
          ...
          return (
            ...
            >
              ...
              <rect
                ...
                fill="transparent"
              />
              {/* Label — using short mapLabel, no "The" */}
              <motion.text
                x={offset + 5}
                y={offset + 13}
