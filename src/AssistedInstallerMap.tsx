  ...
  return (
    <svg viewBox="0 0 800 520" className="rosa-svg">
      ...
      <text x="30" y="42" fontSize="14" fill="#7B1FA2" fontWeight="bold">
        🏗️ {b ? 'Assisted Installer — Step-by-Step Cluster Setup' : <>{tip('OCP')} Assisted Installer Flow</>}
      </text>
      ...
      <text x="30" y="95" fontSize="12" fill="#CC0000" fontWeight="bold">
        🖥️ {b ? 'console.redhat.com/openshift (starting point)' : <>{tip('OCM')} Assisted Installer UI</>}
      </text>
      ...
      <text x="42" y="136" fontSize="7" fill="#777">{b ? 'Name, version, network' : <>Name, base domain, {tip('OCP')} ver</>}</text>
      ...
      <text x="212" y="122" fontSize="9" fill="#C62828" fontWeight="bold">② {b ? 'Download Boot File' : <>Generate Discovery {tip('ISO')}</>}</text>
      <text x="212" y="136" fontSize="7" fill="#777">{b ? 'Get a special file that' : <>{tip('ISO')} with discovery</>}</text>
      <text x="212" y="148" fontSize="7" fill="#777">{b ? 'helps find your servers' : 'agent embedded. Download'}</text>
      <text x="212" y="160" fontSize="7" fill="#777">{b ? '' : <>or mount via {tip('BMC')}/{tip('IPMI')}.</>}</text>
      ...
      <text x="200" y="230" fontSize="8" fill="#7B1FA2" fontWeight="bold">{b ? 'Boot servers with file' : <>Boot hosts from {tip('ISO')}</>}</text>
      ...
      <text x="310" y="328" fontSize="7" fill="#777">{b ? 'enough servers? network ok?' : <>{tip('NTP')}, connectivity, disk space,</>}</text>
      ...
      <text x="50" y="395" fontSize="11" fill="#1565C0" fontWeight="bold">⑤ {b ? 'Your OpenShift Cluster (installed!)' : <>{tip('OCP')} Cluster (Installed)</>}</text>
      ...
      <text x="65" y="468" fontSize="7" fill="#B71C1C">{b ? 'These run the cluster brain' : <>{tip('API')}, {tip('etcd')}, scheduler, controllers</>}</text>
      ...
      {(b ? [
        ...
        <>⬆️ {tip('OCP')} upgrades (oc adm upgrade)</>,
        <>📊 Monitoring stack ({tip('Prometheus')})</>,
        <>🔒 Security patches, {tip('CVE')} response</>,
        <>💡 Datacenter facilities</>,
        <>🌐 DNS (base domain, {tip('API')}, *.apps)</>,
        <>📜 {tip('TLS')} / {tip('PKI')} management</>,
        <>🛡️ {tip('etcd')} backup strategy</>,
        <>{tip('DR')} / {tip('HA')} planning</>,
      ]).map((item, i) => (
        <g key={i}>
    ...
    <div className="rosa-map">
      ...
      <div className="rosa-variant-note" style={{ borderLeft: '4px solid #7B1FA2' }}>
        ...
        <p>{b
          ...
          : <>{g('Ideal for on-premise deployments where managed services aren\'t an option. Supports bare metal, vSphere, and platform-agnostic installations. Connected (internet-facing) or restricted/air-gapped networks. Day-2 operations (upgrades, monitoring, backup) are customer responsibility. Red Hat provides support entitlement.')}</>}
        </p>
      </div>
      ...
      <div className="rosa-variant-note">
        <p>{b
          ...
          : <>📌 Key distinction: No {g('SRE')} management post-install. Customer assumes full operational responsibility. Red Hat support available via case system but no proactive monitoring. Upgrade initiated by customer via `oc adm upgrade` or {g('OCP')} console.</>}
        </p>
      </div>
        ...
        <div className="rosa-bridge-content">
          <div className="rosa-bridge-side">
            ...
            <p>{b
              ...
              : <>{g('Physical hosts with IPMI/BMC/Redfish. Boot via virtual media or PXE. RHCOS on disk. Minimum 5 hosts (3 CP + 2 W) or 3 for compact.')}</>}
            </p>
          </div>
          ...
          <div className="rosa-bridge-side">
            ...
            <p>{b
              ...
              : <>{g('vSphere integration (storage, CCM) or platform=none for any IaaS. Same discovery agent flow. vSphere: CSI driver, cloud-controller-manager integration.')}</>}
            </p>
          </div>
        </div>
      </div>
